...
if __name__ == "__main__":
    import uos as os
    #file_in = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'images_for_test', 'img_LQ_4b_48x48.bmp')
    #print(f'Read BitMap file {file_in} ')
    img = MicroBMP().load("images/img_LQ_4b_48x48.bmp")
    x0 = 0
    y0 = 0
    for y in range(img.DIB_h):
        for x in range(img.DIB_w):
            r, g, b = img.palette[img[x, y]]
            print(x + x0, y + y0, (r, g, b))
